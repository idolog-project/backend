...
- 추천 파이프라인 실패는 HTTP 503을 유지한다. 일반 실패 코드는 RECOMMENDATION_FAILED, Gemini 일반 요청 제한은 AI_RATE_LIMITED, 확인된 일일 한도 초과는 AI_DAILY_LIMIT_EXCEEDED다.
- 일일 한도는 SDK 오류 본문의 google.rpc.QuotaFailure.violations에서 quotaId 또는 quotaMetric에 일일 주기가 명시된 경우에만 분류한다. 불명확한 429나 단순 오류 문구만으로 일일 한도를 추정하지 않는다.
- 60초 호출 차단 중에도 판별한 제한 종류를 유지한다. 이 시간이 일일 한도 초기화를 뜻하지는 않는다. 원본 오류 본문과 프로젝트 정보는 클라이언트에 전달하지 않는다.
